...
class VesperCompiler:
    ...
    def validate(self, node: VesperNode) -> ValidationResult:
        ...
        if not re.match(r"^[a-z_]+_v[0-9]+", node.node_id):
            result.add_error(
                "node_id", f"Invalid node_id format: {node.node_id}. Expected: name_vN"
            )
        ...
        for i, step in enumerate(node.flow):
            ...
            if (
                not step.operation.startswith("custom.")
                and step.operation not in valid_operations
            ):
                result.add_warning(
                    f"flow[{i}].operation", f"Unknown operation: {step.operation}"
                )
    ...
    def compile(self, node: VesperNode) -> str:
        ...
        # Header
        lines.append('"""')
        lines.append("AUTO-GENERATED by Vesper Compiler - DO NOT EDIT")
        lines.append("")
        ...
        if node.contracts.preconditions:
            ...
            for condition in node.contracts.preconditions:
                ...
                lines.append(
                    f'        return Result(error=Error(error_code="precondition_failed", message="{condition}"))'
                )
            lines.append("")
        ...
        lines.append('    """Wrapper with postcondition verification."""')

        # Store old values for postconditions
        lines.append("    # Execute main function")
        call_args = ", ".join(name for name in node.inputs.keys())
    ...
    def _generate_step(
        self, step: FlowStep, context_vars: set[str], indent: int = 1
    ) -> list[str]:
    ...
    def _generate_validation_step(self, step: FlowStep, prefix: str) -> list[str]:
        ...
        for guard in step.guards:
            ...
            if step.on_failure and "return_error" in step.on_failure:
                ...
                lines.append(
                    f'{prefix}    return Result(error=Error(error_code="{error_code}", message="{message}"))'
                )
            else:
                lines.append(
                    f'{prefix}    return Result(error=Error(error_code="validation_failed", message="Guard failed: {guard}"))'
                )

        return lines

    def _generate_template_step(
        self, step: FlowStep, prefix: str, context_vars: set[str]
    ) -> list[str]:
    ...
    def compile_to_file(
        self, source: str | Path, output_path: Path | None = None
    ) -> Path:
        """
